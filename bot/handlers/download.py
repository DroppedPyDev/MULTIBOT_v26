...
from bot.services import converter
...
# file_id/local path per pending conversion, keyed by the callback's message id.
# In-memory is fine for a single-process bot; swap for Redis if you scale to
# multiple instances.
_pending_conversions: dict[str, str] = {}
...
async def handle_incoming_media(message: Message) -> None:
    """User sent a file directly — offer conversion options."""
    file_obj = message.video or message.audio or message.document
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🎵 Extract MP3", callback_data=f"conv:mp3:{file_obj.file_id}"),
            InlineKeyboardButton(text="🎞 To GIF", callback_data=f"conv:gif:{file_obj.file_id}"),
        ],
        [
            InlineKeyboardButton(text="📹 To MP4", callback_data=f"conv:mp4:{file_obj.file_id}"),
        ],
    ])
    await message.answer("What would you like to convert this to?", reply_markup=keyboard)


@router.callback_query(F.data.startswith("conv:"))
async def handle_conversion_choice(callback: CallbackQuery) -> None:
    _, target_format, file_id = callback.data.split(":", 2)
